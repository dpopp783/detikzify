...
def run(screen_width: int, screen_height: int):
    ...
    available_node_styles = DefaultNodeStyles.default_node_styles
    used_node_styles = set()
    ...
    create_node_style_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((625, 25), (150, 50)),
                                            text='New Node Style',
    ...
    row = 0
    col = 0
    for node_style in available_node_styles:
        split_name = node_style.name.split("_")
        ...
        buttons[node_style.name] = create_node_button
        col = (col + 1) % 3
        if col == 0:
            row = (row + 1) % 3

    buttons["create_node_style"] = create_node_style_button
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == buttons["create_node_style"]:
                    print("Clicked Create Node Style")
                    ...
                    for node_style in used_node_styles:
                        tikz_picture.add_node_style(node_style)

                    for node in nodes:
                        tikz_picture.add_node(node.tikz_node)
                    ...
                    cnode_small = DraggableNode([0, 0, 25, 25], canvas, "circle", DefaultNodeStyles.cnode_small)
                    nodes.append(cnode_small)
                    used_node_styles.add(DefaultNodeStyles.cnode_small)
                elif event.ui_element == buttons["cnode_med"]:
                    cnode_med = DraggableNode([0, 0, 50, 50], canvas, "circle", DefaultNodeStyles.cnode_med)
                    nodes.append(cnode_med)
                    used_node_styles.add(DefaultNodeStyles.cnode_med)
                elif event.ui_element == buttons["cnode_large"]:
                    cnode_large = DraggableNode([0, 0, 100, 100], canvas, "circle", DefaultNodeStyles.cnode_large)
                    nodes.append(cnode_large)
                    used_node_styles.add(DefaultNodeStyles.cnode_large)
                elif event.ui_element == buttons["sqnode_small"]:
                    sqnode_small = DraggableNode([0, 0, 25, 25], canvas, "rect", DefaultNodeStyles.sqnode_small)
                    nodes.append(sqnode_small)
                    used_node_styles.add(DefaultNodeStyles.sqnode_small)
                elif event.ui_element == buttons["sqnode_med"]:
                    sqnode_med = DraggableNode([0, 0, 50, 50], canvas, "rect", DefaultNodeStyles.sqnode_med)
                    nodes.append(sqnode_med)
                    used_node_styles.add(DefaultNodeStyles.sqnode_med)
                elif event.ui_element == buttons["sqnode_large"]:
                    sqnode_large = DraggableNode([0, 0, 100, 100], canvas, "rect", DefaultNodeStyles.sqnode_large)
                    nodes.append(sqnode_large)
                    used_node_styles.add(DefaultNodeStyles.sqnode_large)
